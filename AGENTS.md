...
- `calculate_tax_report.py` contains the main tax calculation logic. Option-to-stock assignment matching resolves IBKR symbol variants (exchange-suffix symbols, ticker renames) via conid/ISIN-based symbol equivalence classes (Issue #83); regression coverage lives in `tests/test_underlying_symbol_matching.py`.
  - Assigned futures options (FOP/FSFOP) never touch the stock path. `_collect_future_assignment_adjustments` (module-level helpers around `_FutureAssignmentContext`) resolves the exact chain option sale → FUT delivery BookTrade → CLOSED_LOT → close by timestamp, conid, quantity, multiplier and strike. IBKR embeds the premium in the delivered future's cost basis; the resolver removes only the net premium from the realized FUT result (also cross-year) and keeps the assignment fee as acquisition cost of the future, so the premium is counted once in Topf 2. Any break in the evidence chain becomes a review item (`stillhalter_corrections_dropped`, labels via `get_stillhalter_review_reason_label`), never a silent correction; applied corrections are listed in `audit['future_assignment_corrections']` and added back in the plausibility check. Covered by TC47–TC57 in `tests/test_cross_year_series.py` and `tests/test_plausibility.py`; the real case is the 6EZ4 put in the local audit1 data.
  - Instrument categories and StmtFunds activity codes are routed via module-level tables, not inline literals: `TOPF2_ASSET_CATEGORIES` / `KNOWN_UNROUTED_ASSET_CATEGORIES` for `assetCategory`, and `INCOME_ACTIVITY_CODES` / `KNOWN_IGNORED_ACTIVITY_CODES` / `FEE_ACTIVITY_CODES` / `MANUAL_REVIEW_ACTIVITY_CODES` for `activityCode`. Anything not covered is collected by `register_unrouted_category` / `register_unhandled_activity_code` and surfaced as a review item (`audit['unrouted_asset_categories']`, `audit['unhandled_activity_codes']`) instead of being dropped silently. When adding support for a new category or code, extend the table rather than the branch, and add a case to `tests/test_asset_category_routing.py`. TTAX transaction taxes are the one review-class code with an automated path (`_collect_transaction_tax_adjustments`, covered by `tests/test_transaction_tax.py`): IBKR books the French (0.3 %, 0.4 % since 1 April 2025) and Spanish (0.2 %) financial transaction tax as one daily aggregate per security; its `tradeQuantity` (fallback: the number ending the description) is the day's net purchase quantity, purchases minus same-day sales, because both taxes are levied on the net position ("French Daily Trade Charge Tax HO 170" = fills 73 + 97; "Spanish Daily Trade Charge Tax BSD2 1" for three buys and three sells). For "Daily Trade Charge" rows the amount is distributed pro rata by trade value across the purchase fills of the day (never across sales); other TTAX texts still need a homogeneous group or a single fill confirmed by the quantity; sale-side tax reduces the closing trade, purchase-side tax is carried through CLOSED_LOTs (lot `transactionID` points at the opening fill; several closing fills in the same second share a lot by quantity). An event is applied completely or not at all; anything unconfirmed stays a review item with a reason code, and the reasons are shown to the user (notice, Rechenwege table, TXT). Real cases: Italian derivative tax (per trade) and the French daily aggregate (local audit scenario `u770_2024`). An export without any CLOSED_LOT row (Flex Query "Trades" without the "Closed Lots" level of detail) gets its own review reason `closed_lot_sektion_fehlt` and the notice `closed_lots_missing`, built from `audit['closed_lot_coverage']` (`lot_rows`, `closing_trades`, per-account `closing_trades_without_lots`, all summed in the merge).
- `extract_ibkr_data.py` converts IBKR XML exports into CSV inputs. The quarterly merge (`extract_quarterly_xmls`) deduplicates trades without `tradeID`, CLOSED_LOTs and FX result rows by full-row occurrence count per statement period plus a document hash, never by composite keys such as timestamp/quantity/price (those collapsed real fills: 17 CLOSED_LOTs on one real export); `pnl_summary` rows are keyed by account, category, conid, ISIN, symbol and currency so that re-used symbols do not merge. Regression: `tests/test_quarterly_history_extraction.py`.
- Trade rows without a nonempty `tradeID` must retain their multiplicity: identical timestamps, quantities, prices, or PnL do not prove a duplicate fill. Cross-file history deduplication uses full-row occurrence counts; the calculator must not collapse these preserved fills again. Cover changes through extraction, final values, and exports in the existing dedupe/history/UI tests.
